...
func HandleLambdaEvent(req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	defer db.Close()
	fmt.Println("Success!")
	query, err := db.Query(`DELETE FROM portfolio_builder_schema.Skill_Categories
	WHERE ID = ` + strconv.Itoa(myRequest.ID) + `;`)
	if err != nil {
		panic(err.Error())
	}
	defer query.Close()
	return events.APIGatewayProxyResponse{Body: "Deleted Skills Succesfully", StatusCode: 200}, nil
